...
1. **[Curator Agent Architecture](#curator-agent-architecture)** - Complete knowledge organization workflow
2. **[Domain-Specific Agents](#domain-specific-agent-examples)** - Code Reviewer, Doc Generator, Performance Optimizer
3. **[Error Handling in Agents](#error-handling-in-agents)** - Patterns for failures and ambiguity
4. **[Token Efficiency](#token-efficiency-in-agents)** - Context windows and progressive disclosure
5. **[Agent Development Workflow](#agent-development-workflow)** - From manual process to deployment
6. **[Safe Modification Patterns](#safe-modification-patterns)** - Read-Validate-Write-Verify pattern
7. **[Agent Composition](#agent-composition-multiple-agents)** - Orchestrating multiple agents
8. **[Deployment Checklist](#deployment-checklist)** - Pre-deployment verification
...
- Use read-only tools only (no Write/Edit)
...
When you are invoked:
- Code profiling needed
- Performance analysis requested
- Optimization planning required
